...
public:
	...
	void PlayMusic(std::string filename,int music_type,int loops = -1);
	void PauseMusic();
	void ResumeMusic();
	void QuitMusic();
	void QuitSFX(int channel);
	...
	void PlaySFX(std::string filename, int sfx_type ,int loops = 0, int channel = 0);
